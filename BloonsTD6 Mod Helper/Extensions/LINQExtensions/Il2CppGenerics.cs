...
public static class Il2CppGenerics
{
    ...
    /// <param name="source"></param>
    /// <returns></returns>
    public static bool Any<T>(this List<T> source) where T : Object => source.Count > 0;
    ...
    /// <param name="source"></param>
    /// <returns></returns>
    public static T Last<T>(this List<T> source) => source.Cast<IEnumerable<T>>().Last();
    ...
    public static T LastOrDefault<T>(this List<T> source, System.Func<T, bool> predicate) =>
        source.Cast<IEnumerable<T>>().Where(predicate).LastOrDefault();
    ...
    /// <param name="source"></param>
    /// <returns></returns>
    public static T First<T>(this List<T> source) => source.Cast<IEnumerable<T>>().First();
    ...
    /// <param name="source"></param>
    /// <returns></returns>
    public static T FirstOrDefault<T>(this List<T> source) => source.Cast<IEnumerable<T>>().FirstOrDefault();
}