...
public static class AbilityModelBehaviorExt
{
    ...
    public static bool HasBehavior<T>(this AbilityModel model) where T : Model => ModelBehaviorExt.HasBehavior<T>(model);

    /// <inheritdoc cref="ModelBehaviorExt.GetBehavior{T}(Il2CppAssets.Scripts.Models.Model)" />
    public static T GetBehavior<T>(this AbilityModel model) where T : Model => ModelBehaviorExt.GetBehavior<T>(model);
