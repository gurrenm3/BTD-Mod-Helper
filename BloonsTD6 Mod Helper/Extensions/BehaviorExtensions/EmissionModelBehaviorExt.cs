...
public static class EmissionModelBehaviorExt
{
    ...
    public static bool HasBehavior<T>(this EmissionModel model) where T : Model => ModelBehaviorExt.HasBehavior<T>(model);

    /// <inheritdoc cref="ModelBehaviorExt.GetBehavior{T}(Il2CppAssets.Scripts.Models.Model)" />
    public static T GetBehavior<T>(this EmissionModel model) where T : Model => ModelBehaviorExt.GetBehavior<T>(model);
