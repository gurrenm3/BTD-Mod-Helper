...
public static class FileInfoExt
{
    ...
    {
        try { return Assembly.Load(fileInfo.FullName).GetReferencedAssemblies(); }
        catch (Exception ex)
        {
            if (ex.Message.Contains("HRESULT: 0x80131515"))
            {
                ModHelper.Log(
                    $"The file you tried accessing is blocked. Please Unblock \"{fileInfo.FullName}\" to continue." +
