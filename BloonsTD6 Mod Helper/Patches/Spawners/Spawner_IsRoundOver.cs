...
internal static class Spawner_IsRoundOver
{
    ...
    private static bool Prefix(Spawner __instance, ref bool __result)
    {
        try
        {
            ...
            var activeBoss = aliveBloons.Exists(bloon => bloon is not null && ModBoss.Cache.ContainsKey(bloon.bloonModel.name) && !ModBoss.Cache[bloon.bloonModel.id].BlockRounds);

            if (activeBoss && !aliveBloons.Exists(bloon => bloon is not null && !ModBoss.Cache.ContainsKey(bloon.bloonModel.name)))
            {
